...
NSString *const ELAsyncUdpSocketException = @"AsyncUdpSocketException";
NSString *const ELAsyncUdpSocketErrorDomain = @"AsyncUdpSocketErrorDomain";
...
@interface ELAsyncUdpSocket (Private)
...
**/
@interface ELAsyncSendPacket : NSObject
{
@public
	NSData *buffer;
	NSData *address;
...
@implementation ELAsyncSendPacket
...
**/
@interface ELAsyncReceivePacket : NSObject
{
@public
	NSTimeInterval timeout;
	long tag;
...
@implementation ELAsyncReceivePacket
...
@implementation ELAsyncUdpSocket
...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
	...
	{
		[NSException raise:ELAsyncUdpSocketException
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	if(errPtr)
	{
		...
		*errPtr = [NSError errorWithDomain:ELAsyncUdpSocketErrorDomain
...
- (BOOL)enablePortReUse:(BOOL)flag error:(NSError **)errPtr
{
    if (theSocket4)
    {
        int value = flag ? 1 : 0;
        ...
        if(error)
        {
            if(errPtr)
            {
                NSString *errMsg = @"Unable to enable port reuse";
                NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:ELAsyncUdpSocketErrorDomain code:AsyncUdpSocketCFSocketError userInfo:info];
}
...
- (NSError *)getIPv4UnavailableError
{
    ...
    return [NSError errorWithDomain:ELAsyncUdpSocketErrorDomain code:AsyncUdpSocketIPv4Unavailable userInfo:info];
}
...
- (NSError *)getIPv6UnavailableError
{
	...
	return [NSError errorWithDomain:ELAsyncUdpSocketErrorDomain code:AsyncUdpSocketIPv6Unavailable userInfo:info];
}
...
- (NSError *)getSendTimeoutError
{
	...
	return [NSError errorWithDomain:ELAsyncUdpSocketErrorDomain code:AsyncUdpSocketSendTimeoutError userInfo:info];
}
- (NSError *)getReceiveTimeoutError
{
	...
	return [NSError errorWithDomain:ELAsyncUdpSocketErrorDomain code:AsyncUdpSocketReceiveTimeoutError userInfo:info];
}
...
- (BOOL)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ELAsyncSendPacket *packet = [[ELAsyncSendPacket alloc] initWithData:data address:nil timeout:timeout tag:tag];
             ...
             tag:(long)tag
{
	...
	ELAsyncSendPacket *packet = nil;
	
	if(address4 && theSocket4)
		packet = [[ELAsyncSendPacket alloc] initWithData:data address:address4 timeout:timeout tag:tag];
	else if(address6 && theSocket6)
		packet = [[ELAsyncSendPacket alloc] initWithData:data address:address6 timeout:timeout tag:tag];
	else
		return NO;
...
- (BOOL)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ELAsyncSendPacket *packet = [[ELAsyncSendPacket alloc] initWithData:data address:remoteAddr timeout:timeout tag:tag];
...
- (CFSocketRef)socketForPacket:(ELAsyncSendPacket *)packet
{
	if(!theSocket4)
		return theSocket6;
	if(!theSocket6)
...
- (void)scheduleDequeueSend
{
	if((theFlags & kDequeueSendScheduled) == 0)
	{
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            [self performSelector:@selector(maybeDequeueSend) withObject:nil afterDelay:0 inModes:self->theRunLoopModes];
...
- (void)receiveWithTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ELAsyncReceivePacket *packet = [[ELAsyncReceivePacket alloc] initWithTimeout:timeout tag:tag];
...
static void MyCFSocketCallback(CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	@autoreleasepool {
		...
		ELAsyncUdpSocket *theSocket = (__bridge ELAsyncUdpSocket *)pInfo;
